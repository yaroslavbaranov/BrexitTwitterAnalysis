require(tm) 
# text mining package -- for document text matrix etc
require(stringr) 
...
setwd('/home/yaroslav/Documents/ucheba/data_analysis/Yaroslav/web_text/BrexitTwitterAnalysis')
...
# Detect/Drop campaign IDs 
    ...
    short_hist = (x$user_hist > as.POSIXlt(strptime("May 23 2016", 
                                                    "%b %d %Y", tz ="GMT")))
    t = table(short_hist)
    ...
    barplot(height = table(x$tweet_time_bin), xlab = "Time", ylab = "# tweets", 
            main = "Streaming Volume", col = 'blue')
...
sprintf("percentage of new account: %.2f", t[2]/t[1]*100)
    ...
    sprintf("Percentage of retweets: %.2f", 100*(x$is_rt %>% 
                                                   table())[2]/length(x$is_rt))
    sprintf("Percentage of unique tweets: %.2f", 
            100*length(unique(x$text))/length(x$text))
    ...
    sprintf("Most frequently retweeted twitterIDs: %s", 
            names(fr_rt)[1:10] %>% str_c(collapse = ", "))
    
    fr_mt = allquoted %>% table() %>% sort(decreasing = T)
    sprintf("Most frequently mentioned twitterIDs: %s", 
            (fr_mt %>% names())[1:10] %>% str_c(collapse = ", "))
    
    pal2 = brewer.pal(8, "Dark2")
    png('fre_retweet_screennames_new.png', 
    ...
    wordcloud(names(tf), tf, random.order=F, scale=c(4,0.5), 
              colors=pal2, max.words = 500)
    dev.off()
    
    ##find associations for politician names, political issues, and emotions
    politician = findAssocs(tu_dtm, c("boris","farage","cameron"),0.8)
